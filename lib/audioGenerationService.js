...
class AudioGenerationService {
  ...
  async generateAudio(text, language = 'fr', provider = 'auto') {
    try {
      ...
      // Web Speech API seulement côté client
      if (!audioBuffer && (provider === 'auto' || provider === 'webspeech') && typeof window !== 'undefined') {
      ...
      if (!audioBuffer) {
        // Si on est côté serveur et que les APIs ont échoué, retourner une erreur explicite
        if (typeof window === 'undefined') {
          throw new Error('APIs TTS non configurées. Veuillez configurer ELEVENLABS_API_KEY ou OPENAI_API_KEY');
        } else {
